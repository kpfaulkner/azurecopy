...
namespace azurecopy
{
    public class OneDriveHandler : IBlobHandler
    {
        ...
        private static Datatypes.OneDriveDirectory destinationDirectory = null;
        private IOneDriveHelper oneDriveHelper;

        public OneDriveHandler(IOneDriveHelper oneDriveHelper, string url )
        {
            this.oneDriveHelper = oneDriveHelper;
            accessToken = oneDriveHelper.GetAccessToken();
            baseUrl = url;
        }
        ...
        public void MakeContainer(string containerName)
        {
            ...
            url = url.Replace(oneDriveHelper.OneDrivePrefix(), "");
            var targetDirectory = oneDriveHelper.CreateFolder(url);
        }
        ...
        public Blob ReadBlob(string containerName, string blobName, string cacheFilePath = "")
        {
            ...
            if (url.IndexOf(oneDriveHelper.OneDrivePrefix()) != -1)
            {
                url = url.Replace(oneDriveHelper.OneDrivePrefix(), "");
                var skydriveFileEntry = oneDriveHelper.GetOneDriveEntryByFileNameAndDirectory(url);
        ...
        public void WriteBlob(string containerName, string blobName, Blob blob, int parallelUploadFactor = 1, int chunkSizeInMB = 4)
        {
            ...
            if (destinationDirectory == null)
            {
                ...
                // if not, create it.
                destinationDirectory = oneDriveHelper.GetOneDriveDirectory(containerName);

                if (destinationDirectory == null)
                {
                    destinationDirectory = oneDriveHelper.CreateFolder(url);
                }
            }
        ...
        {
            var skydriveListing = oneDriveHelper.ListOneDriveDirectoryContent(containerName);
            foreach (var skyDriveEntry in skydriveListing)
            {
                var blob = new BasicBlobContainer();
                blob.Name = skyDriveEntry.Name;

                var resolvedOneDriveEntry = oneDriveHelper.ListOneDriveFileWithUrl(skyDriveEntry.Id);
        ...
        {
            var skydriveListing = oneDriveHelper.ListOneDriveRootDirectories();
